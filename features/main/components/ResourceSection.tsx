    ...
    return (
        <section className="bg-slate-50 pb-32 pt-16 px-10 w-full text-left relative overflow-hidden">
            ...
            <div className="mx-auto max-w-[1200px] relative z-10">
                <div className="bg-white/60 backdrop-blur-xl rounded-[3rem] p-10 md:p-16 shadow-[0_20px_50px_rgba(0,0,0,0.15)] border-2 border-white/40 transition-all duration-500 hover:shadow-[0_25px_60px_rgba(16,185,129,0.15)]">
                    <h2 className="text-4xl font-black text-slate-900 text-center mb-16 tracking-tighter drop-shadow-sm">연구소 자료실</h2>
                    <div className="flex flex-col gap-3">
                        {posts.length > 0 ? (
                            posts.map((post) => (
                                <Link href={`/archive/${post.id}`} key={post.id} className="group bg-white/50 backdrop-blur-md rounded-2xl p-5 flex flex-col md:flex-row items-start md:items-center justify-between border border-white/30 shadow-md hover:bg-white/80 hover:border-emerald-200 hover:shadow-[0_12px_30px_rgba(16,185,129,0.1)] hover:-translate-y-1 transition-all duration-300">
