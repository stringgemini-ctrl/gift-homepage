...
export default function ResourceSection({ posts, isLoading }: Props) {
    return (
            ...
            <div className="mx-auto max-w-[1200px] relative z-10">
                <div className="bg-white/70 backdrop-blur-2xl rounded-[3rem] p-10 md:p-16 shadow-[0_8px_40px_rgba(0,0,0,0.04)] border border-white transition-all duration-500 hover:shadow-[0_20px_50px_rgba(16,185,129,0.08)]">
                    ...
                    <div className="flex flex-col gap-3">
                        {isLoading ? (
                            <div className="py-12 text-center text-slate-500 font-medium bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">자료를 불러오는 중입니다.</div>
                        ) : posts.length > 0 ? (
                            posts.map((post) => (
